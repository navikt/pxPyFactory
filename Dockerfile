FROM europe-north1-docker.pkg.dev/cgr-nav/pull-through/nav.no/python:3.11.0-dev AS dev
USER root
...
ENV PIP_TARGET=/pyfactory/lib/python3.11/site-packages
...
COPY run.py .
COPY --from=dev /pyfactory/lib/python3.11/site-packages/ /pyfactory/lib/python3.11/site-packages/
COPY --from=dev /etc/passwd /etc/passwd
USER pyfactory
ENV PYTHONPATH=/pyfactory/lib/python3.11/site-packages
