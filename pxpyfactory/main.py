...
def main():
    ...
    contvariable = 'cont_var' # Column name for contents variable - can probably be anything..
    ...
    data_products.rename(columns={'OMRAADE': 'LEVEL_1'}, inplace=True)
    data_products.rename(columns={'TEMA_MAPPE': 'LEVEL_2_FOLDER'}, inplace=True)
    data_products.rename(columns={'TEMA': 'LEVEL_2'}, inplace=True)
    ...
    print(data_products[['LEVEL_1_FOLDER', 'LEVEL_2_FOLDER', 'TABLE_NO', 'TITLE']])
    ...
    for idx, row in data_products.iterrows():
        ...
        px_output_path  = os.path.abspath(os.path.join(output_path, row['LEVEL_1_FOLDER'], row['LEVEL_2_FOLDER'], ('TN_' + table_ref + '.px')))
        ...
        values_dict = {}
        # values_dict[contvariable] = data_list
        ...
        data_out = prepare_data_matrix(values_dict, stub_list, heading_list, data_list, table_data).fillna(str('"' + fill_value + '"'))
        ...
        list_of_lines = prepare_px_lines(meta_out, data_out)
